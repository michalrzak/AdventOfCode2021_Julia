...
function find_solved(mask, solved)
...
function main()
    ...
    solved = zeros(Bool, size(fields, 1))
    last_result = -1
    last_number = -1
    for number in drawings
        ...
        while result != -1
            result = find_solved(mask, solved)
    ...
    println(get_solution(fields[last_result, 1:end, 1:end], mask[last_result, 1:end, 1:end]) * last_number)
    println(last_result)
    println(mask[last_result, 1:end, 1:end])
