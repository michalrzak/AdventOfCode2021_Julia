...
"""
The code technically works, it just takes a bit to finnish.
"""

function main()
    input = [[parse(Int, c) for c in line] for line in eachline("input.txt")]
    cost_tile = hcat(input...)'
    costs = []
    
    for x in 0:4
        col = []
        for y in 0:4
            new_tile = (cost_tile .+ x .+ y)
            new_tile[new_tile .>= 10] .+= 1
            new_tile[new_tile .>= 10] .%= 10
            if col == []
                col = new_tile
            else
                col = hcat(col, new_tile)
            end
        end
        if costs == []
            costs = col
        else
            costs = vcat(costs, col)
        end
    end
