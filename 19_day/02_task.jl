...
function get_m_distance(v1, v2)
    return abs(v1[1] - v2[1]) + abs(v1[2] - v2[2]) + abs(v1[3] - v2[3])
end
...
function main()
    ...
    for i in 1:length(centers)
        for j in i+1:length(centers)
            m_dist = get_m_distance(centers[i], centers[j])
            if m_dist > res_dist
                res_dist = m_dist
            end
        end
